...
const SectionArena = () => {
        ...
        if (sec) {
            for (let i = 0; i < sec.length; i++) {
                for (let j = 0; j < sec[i].taskIDList.length; j++) {
                    sec[i].taskIDList[j] = task.find(element => element.id === sec[i].taskIDList[j])
                }
            }
        }
    ...
    useEffect(() => {
        fetch('http://localhost:8000/sectionList')
            ...
            .then(async (sec) => {
                // console.log(sec);
                const task = await fetch("http://localhost:8000/taskList")
                    .then((res) => {
                        // console.log(res);
                        if (!res.ok) {
    ...
    function findIndex(sectionTaskList, taskId) {
        for (let i = 0; i < sectionTaskList.length; i++) {
            if (sectionTaskList[i].id == taskId)
                return i;
        }
    }
    ...
    const handleOnDragEnd = (result) => {
        ...
        if (destination.droppableId === source.droppableId && destination.index === source.index) {
            ...
            setSectionList(prev => {
                ...
                let indexofSource = findIndex(temp[source.droppableId].taskIDList, parseInt(source.index));
                let indexofDestination = findIndex(temp[destination.droppableId].taskIDList, parseInt(destination.index));
                temp[source.droppableId].taskIDList = changePos(temp[source.droppableId].taskIDList, indexofSource, indexofDestination);
            ...
            setSectionList(prev => {
                ...
                let indexofSource = findIndex(temp[source.droppableId].taskIDList, parseInt(source.index));
                let indexofDestination = findIndex(temp[destination.droppableId].taskIDList, parseInt(destination.index));
                changePosInDiffArray(temp[source.droppableId].taskIDList, temp[destination.droppableId].taskIDList, indexofSource, indexofDestination);
                // temp[source.droppableId] = temp[source.droppableId];
                // console.log("after swap", temp);
                return temp;
            });
        ...
        <>
            <DragDropContext onDragEnd={handleOnDragEnd}>
                <div className="section-arena">
                    ...
                    <div className='add-section-div' onClick={createSection} ><img className="add-section-img" src="https://img.icons8.com/sf-regular/48/000000/add.png" /><p className='paraChanges'>Add Section</p></div>
                </div>
            </DragDropContext>
            {/* {selectedTask != null && 
